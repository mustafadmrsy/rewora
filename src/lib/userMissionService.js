...
export async function listUserMissions(pageOrUrl = null) {
  ...
  console.log('listUserMissions - raw response:', res)

  // Response: { success: 200, data: { user_missions: {...} } }
  const responseData = res?.data ?? res
  console.log('listUserMissions - responseData:', responseData)

  const userMissionsPayload = responseData?.user_missions ?? {}
  console.log('listUserMissions - userMissionsPayload:', userMissionsPayload)

  const list = Array.isArray(userMissionsPayload?.data)
    ? userMissionsPayload.data
    : toArray(userMissionsPayload)

  console.log('listUserMissions - list:', list)
  console.log('listUserMissions - list length:', list.length)

  const userMissions = list.map(mapUserMission).filter(Boolean)

  console.log('listUserMissions - userMissions:', userMissions)
  console.log('listUserMissions - userMissions length:', userMissions.length)
